...
### 选择器规范
- 当构建选择器时应该使用清晰，准确和有语义的 class 名。尽量减少使用标签选择器；
 ...
 ```
- 使用提供的简写属性（font、background等等）
- 为选择器中的属性添加双引号 不允许单引号
     - 例如，input[type="text"]。只有在某些情况下是可选的，但是，为了代码的一致性，建议都加上双引号。
- 选择器的嵌套层级应不大于 3 级，位置靠后的限定条件应尽可能精确，避免使用不必要的嵌套，只有在必须将样式限制在父元素内（也就是后代选择器），并且存在多个需要嵌套的元素时才使用嵌套。
